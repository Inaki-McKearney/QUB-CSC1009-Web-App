...
import views.SignUpSubmit;
import views.AddSongSubmit;
...
public class Main 
{
	...
	{		
		...
		registerShutdownHook(databaseInterface);		System.out.println("Project:csc9");

		ContactUs iContactUs = new ContactUs(databaseInterface,fileStoreInterface);

		PersonalPage iPersonalPage = new PersonalPage(databaseInterface,fileStoreInterface);

		ResultsPage iResultsPage = new ResultsPage(databaseInterface,fileStoreInterface);

		LandingPage iLandingPage = new LandingPage(databaseInterface,fileStoreInterface);

		SignUpSubmit iSignUpSubmit = new SignUpSubmit(databaseInterface,fileStoreInterface);

		AddSongSubmit iAddSongSubmit = new AddSongSubmit(databaseInterface,fileStoreInterface);
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
		        ...
		        //the example dynamic page will process the request if the index.html file is not present
		        	if(toProcess.path.length()==0)
		        		toProcess.path = "index.html";		if(iContactUs.process(toProcess))
		{
		}
		else
		if(iPersonalPage.process(toProcess))
		{
		}
		else
		if(iResultsPage.process(toProcess))
		{
		}
		else
		if(iLandingPage.process(toProcess))
		{
		}
		else
		if(iSignUpSubmit.process(toProcess))
		{
		}
		else
		if(iAddSongSubmit.process(toProcess))
		{
		}
		else
		        {
